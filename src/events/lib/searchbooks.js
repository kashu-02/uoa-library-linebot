...
export default async (searchWord, searchIndex) => {
  const { data } = await axios.get(
    `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(
      searchWord
    )}&maxResults=4&startIndex=${
      searchIndex || 0
    }&fields=totalItems,items(volumeInfo,saleInfo/isEbook)`
  );

  if (!data || data.totalItems === 0) {
    // If google API returns empty (= not hit).
    return 'No books.';
  }
  console.log(`totalitem: ${data.totalItems}\n\n`);
  console.log(`datas: ${JSON.stringify(data)}`);
  let isEnd = false;
  if (data.totalItems === searchIndex - 4) {
    // The provided information
    isEnd = true;
  }

  const filtered = data.items.filter((item) => {
    // Filter books which are NOT Ebook and have ISBN-13 number.
    if (
      !item.saleInfo ||
      !item.volumeInfo ||
  ...
  const result = await Promise.all(
    filtered.map(async (element, index) => {
      console.log(`map: ${index}`);
      let author;
      if (!element.volumeInfo?.authors) {
        // If authors info is not served
        author = '';
      } else if (element.volumeInfo.authors.length >= 2) {
      ...
      const ISBN_13 = element.volumeInfo.industryIdentifiers.find(
        (element1) => element1.type === 'ISBN_13'
      ).identifier;
      return {
        ...
        thumbnailLink: element.volumeInfo?.imageLinks?.thumbnail
          ? await getThumbnailLink(
